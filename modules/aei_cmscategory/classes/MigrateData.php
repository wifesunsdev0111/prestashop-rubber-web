...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'aeicmscategoryinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'aeicmscategoryinfo_lang` il ON il.`id_aeicmscategoryinfo` = i.`id_aeicmscategoryinfo`'
        );
    ...
    public function insertData()
    {
        ...
        $AEICmsCategorys = array_intersect_key($this->loadedData, $shopsIds);

        $aeicmscategoryinfo = new CustomText();
        $aeicmscategoryinfo->text = reset($AEICmsCategorys);
        $return &= $aeicmscategoryinfo->add();

        if (count($AEICmsCategorys) > 1) {
            foreach ($AEICmsCategorys as $key => $text) {
                Shop::setContext(Shop::CONTEXT_SHOP, (int) $key);
                $aeicmscategoryinfo->text = $text;
                $return &= $aeicmscategoryinfo->save();
            }
        }
