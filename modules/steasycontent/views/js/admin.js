...
jQuery(function($){
	...
	checkEasyContetHookInto($('#easy_content_location').val());
});
...
function checkEasyContetHookInto(hookinto)
{
    if(hookinto==3 || hookinto==12 || hookinto==13)
    {
        $('#easy_content_span').attr('disabled',false);
        troggleForm('easy_content_span', true);
    }
    else
    {
        $('#easy_content_span').val(0).attr('disabled',true);
        troggleForm('easy_content_span', false);
    }

    if (hookinto == 4)
    {
        $('#filter').attr('disabled',false);
        troggleForm('filter', true);
    }  
    else
    {
        $('#filter').attr('disabled',true);
        troggleForm('filter', false);
    }
    if (hookinto == 25 || hookinto == 26)
    {
        $('#filter2').attr('disabled',false);
        troggleForm('filter2', true);
    }
    else
    {
        $('#filter2').attr('disabled',true);
        troggleForm('filter2', false);
    }
}

function troggleForm(_this, flag)
{
    if (flag)
    {
        $('#'+_this).parents('.form-group').show();
    }
    else
    {
