...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'aeicmstestimonyinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'aeicmstestimonyinfo_lang` il ON il.`id_aeicmstestimonyinfo` = i.`id_aeicmstestimonyinfo`'
        );
    ...
    public function insertData()
    {
        ...
        $AEICmsTestimonys = array_intersect_key($this->loadedData, $shopsIds);

        $aeicmstestimonyinfo = new CustomText();
        $aeicmstestimonyinfo->text = reset($AEICmsTestimonys);
        $return &= $aeicmstestimonyinfo->add();

        if (count($AEICmsTestimonys) > 1) {
            foreach ($AEICmsTestimonys as $key => $text) {
                Shop::setContext(Shop::CONTEXT_SHOP, (int) $key);
                $aeicmstestimonyinfo->text = $text;
                $return &= $aeicmstestimonyinfo->save();
            }
        }
