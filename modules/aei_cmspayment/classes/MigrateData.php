...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'aeicmspaymentinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'aeicmspaymentinfo_lang` il ON il.`id_aeicmspaymentinfo` = i.`id_aeicmspaymentinfo`'
        );
    ...
    public function insertData()
    {
        ...
        $AEICmsPayments = array_intersect_key($this->loadedData, $shopsIds);

        $aeicmspaymentinfo = new CustomText();
        $aeicmspaymentinfo->text = reset($AEICmsPayments);
        $return &= $aeicmspaymentinfo->add();

        if (count($AEICmsPayments) > 1) {
            foreach ($AEICmsPayments as $key => $text) {
                Shop::setContext(Shop::CONTEXT_SHOP, (int) $key);
                $aeicmspaymentinfo->text = $text;
                $return &= $aeicmspaymentinfo->save();
            }
        }
