...
<div class="row">
    <div class="col-xs-12">
        <p class="payment_module" onclick="document.forms['ogone_form'].submit(); return false;" >
            <a class="ogone"  href="#" title="{l s='Pay with Ingenico ePayments' mod='ogone'}">
                <img src="{$default_option_logo|escape:'htmlall':'UTF-8'}"/>
                {if isset($default_option_name) && $default_option_name}
                    {$default_option_name|escape:'htmlall':'UTF-8'}
                {else}
                    {l s='Pay with Ingenico ePayments' mod='ogone'}<span> ({l s='Pay safely on the next page with Ingenico using your preferred payment method' mod='ogone'})</span>
                {/if}
            </a>
        </p>
    </div>
