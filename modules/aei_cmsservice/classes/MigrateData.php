...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'aeicmsserviceinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'aeicmsserviceinfo_lang` il ON il.`id_aeicmsserviceinfo` = i.`id_aeicmsserviceinfo`'
        );
    ...
    public function insertData()
    {
        ...
        $AEICmsServices = array_intersect_key($this->loadedData, $shopsIds);

        $aeicmsserviceinfo = new CustomText();
        $aeicmsserviceinfo->text = reset($AEICmsServices);
        $return &= $aeicmsserviceinfo->add();

        if (count($AEICmsServices) > 1) {
            foreach ($AEICmsServices as $key => $text) {
                Shop::setContext(Shop::CONTEXT_SHOP, (int) $key);
                $aeicmsserviceinfo->text = $text;
                $return &= $aeicmsserviceinfo->save();
            }
        }
