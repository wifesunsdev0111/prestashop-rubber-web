...
class AEILeftBanner extends ObjectModel
{
	public $title;
	public $url;
	public $image;
	...
	public static $definition = array(
		'table' => 'aeileftbanner_slides',
		'primary' => 'id_aeileftbanner_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'aeileftbanner` (`id_shop`, `id_aeileftbanner_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'aeileftbanner`
			WHERE `id_aeileftbanner_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_aeileftbanner_slides` = hs.`id_aeileftbanner_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_aeileftbanner_slides` as id_slide
			FROM `'._DB_PREFIX_.'aeileftbanner_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'aeileftbanner` hs ON (hss.`id_aeileftbanner_slides` = hs.`id_aeileftbanner_slides`)
