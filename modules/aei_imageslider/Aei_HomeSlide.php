...
class Aei_HomeSlide extends ObjectModel
{
	public $title;
	public $description;
	public $url;
	...
	public static $definition = array(
		'table' => 'aeihomeslider_slides',
		'primary' => 'id_aeihomeslider_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'aeihomeslider` (`id_shop`, `id_aeihomeslider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'aeihomeslider`
			WHERE `id_aeihomeslider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_aeihomeslider_slides` = hs.`id_aeihomeslider_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_aeihomeslider_slides` as id_slide
			FROM `'._DB_PREFIX_.'aeihomeslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'aeihomeslider` hs ON (hss.`id_aeihomeslider_slides` = hs.`id_aeihomeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new Aei_HomeSlide($row['id_slide']);
