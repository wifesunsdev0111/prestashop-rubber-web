	...
	public function renderContent(  $args, $setting ){
        ...
        foreach($catids as $cate_id){
            if(isset($setting["cate_depth"]) && ($setting["cate_depth"] || $setting["cate_depth"] == '0')){
                ...
                $sql_filter = ' AND c.`level_depth` <= ' .(int)$setting["cate_depth"] . ' + (select c.`level_depth` from `ps_category` c where c.id_category ='. $cate_id .')';
            }
            $result_cate = $this->getNestedCategories(
                        $images, $cate_id, $setting["id_lang"], false, null, true, $sql_filter, $sql_sort);
            $result[] = $result_cate;
