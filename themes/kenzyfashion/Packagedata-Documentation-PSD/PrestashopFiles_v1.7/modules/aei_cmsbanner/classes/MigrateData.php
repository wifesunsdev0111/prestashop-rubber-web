...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'aeicmsbannerinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'aeicmsbannerinfo_lang` il ON il.`id_aeicmsbannerinfo` = i.`id_aeicmsbannerinfo`'
        );
    ...
    public function insertData()
    {
        ...
        $AEICmsBanners = array_intersect_key($this->loadedData, $shopsIds);

        $aeicmsbannerinfo = new CustomText();
        $aeicmsbannerinfo->text = reset($AEICmsBanners);
        $return &= $aeicmsbannerinfo->add();

        if (count($AEICmsBanners) > 1) {
            foreach ($AEICmsBanners as $key => $text) {
                Shop::setContext(Shop::CONTEXT_SHOP, (int) $key);
                $aeicmsbannerinfo->text = $text;
                $return &= $aeicmsbannerinfo->save();
            }
        }
