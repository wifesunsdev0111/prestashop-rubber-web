...
include_once('aei_imageslider.php');

$home_slider = new Aei_ImageSlider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'aeihomeslider_slides` SET `position` = '.(int)$position.'
			WHERE `id_aeihomeslider_slides` = '.(int)$id_slide
		);
