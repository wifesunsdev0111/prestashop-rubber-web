...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'aeicmstopinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'aeicmstopinfo_lang` il ON il.`id_aeicmstopinfo` = i.`id_aeicmstopinfo`'
        );
    ...
    public function insertData()
    {
        ...
        $AEICmsTops = array_intersect_key($this->loadedData, $shopsIds);

        $aeicmstopinfo = new CustomText();
        $aeicmstopinfo->text = reset($AEICmsTops);
        $return &= $aeicmstopinfo->add();

        if (count($AEICmsTops) > 1) {
            foreach ($AEICmsTops as $key => $text) {
                Shop::setContext(Shop::CONTEXT_SHOP, (int) $key);
                $aeicmstopinfo->text = $text;
                $return &= $aeicmstopinfo->save();
            }
        }
