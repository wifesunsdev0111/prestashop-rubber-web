				...
				{if isset($images)}
					{foreach from=$images item=image name=thumbnails}
						...
						<li id="thumbnail_{$image.id_image}">
							<a href="{$link->getImageLink($product.link_rewrite, $imageIds, 'thickbox_default')|escape:'html'}" rel="other-views" class="thickbox-ajax-{$product.id_product}{if $smarty.foreach.thumbnails.first} shown{/if}" title="{$image.legend|htmlspecialchars}">
