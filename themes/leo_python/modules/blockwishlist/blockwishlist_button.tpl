	...
	<div class="wishlist">
		{foreach name=wl from=$wishlists item=wishlist}
			{if $smarty.foreach.wl.first}
				<a class="wishlist_button_list btn btn-outline" data-link="{$product.id_product|intval}" tabindex="0" data-toggle="popover" data-trigger="focus" title="{l s='Wishlist' mod='blockwishlist'}" data-placement="top">
...
{else}
		...
		<a class="btn btn-outline addToWishlist wishlistProd_{$product.id_product|intval}" data-link="{$product.id_product|intval}" href="#" onclick="WishlistCart('wishlist_block_list', 'add', '{$product.id_product|intval}', false, 1); return false;" title="{l s='Add to my wishlist' mod='blockwishlist'}">
			<i class="fa fa-heart"></i> <span>{l s="Add to Wishlist" mod='blockwishlist'}</span>
		</a>	
