	...
	<div class="primary_block row">
		...
		<div class="pb-left-column col-xs-12 col-sm-12 col-md-5">
			...
			{if isset($images) && count($images) > 0}
				<!-- thumbnails -->
				<div id="views_block" class="clearfix {if (isset($images) && count($images) < 2) || isset($attribute_images) && count($attribute_images)}hidden{/if}">
			...
			{if isset($images) && count($images) > 1}
				<p class="resetimg clear no-print {if isset($attribute_images) && count($attribute_images)}hidden{/if}">
		...
		<div class="pb-right-column col-xs-12 col-sm-12 col-md-7">
			...
			<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
				...
				<div class="box-info-product  clearfix">
									...
									{if $group.attributes|@count}
										<fieldset class="attribute_fieldset">
											...
											<div class="attribute_list">
												...
												{if isset($attribute_images[$id_attribute_group])}
													<ul class="image-attribute-list">
														{foreach from=$group.attributes key=id_attribute item=group_attribute}
															<li>
																<input id="{$groupName}_{$id_attribute}" type="radio" class="attribute_radio attribute_radio_{$group['group_name']}" name="{$groupName|escape:'html':'UTF-8'}" value="{$id_attribute}" {if ($group.default == $id_attribute)} checked="checked"{/if} {*{if $group.attributes_quantity[$id_attribute] <= 0} disabled {/if} *} data-text="{$group.name|escape:'html':'UTF-8'} {$group_attribute|escape:'html':'UTF-8'}"/>
																<label for="{$groupName}_{$id_attribute}">
																	{if isset($attribute_images[$id_attribute_group][$id_attribute]) && $attribute_images[$id_attribute_group][$id_attribute] > 0}
																		<img src="{$link->getImageLink($group_attribute, $attribute_images[$id_attribute_group][$id_attribute], 'small_default')|escape:'html':'UTF-8'}" alt="{$group_attribute|escape:'html':'UTF-8'}" title="{$group_attribute|escape:'html':'UTF-8'}"/>
																	{else}
																		{$group_attribute|escape:'html':'UTF-8'}
																	{/if}
																</label>
																<div style="text-align: center">{$group_attribute|escape:'html':'UTF-8'}</div>
															</li>
														{/foreach}
													</ul>
					...
					<div class="box-cart box-cart-bottom">
						<div class="btncart" {if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
			...
			<div class="category_tags">
				{hook h="displayProductInformation" product=$product}
			</div>
