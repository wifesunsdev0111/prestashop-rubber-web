...
<html>
	...
	<body>
		...
		<p>Via deze plugin kunt u <a href="http://www.ideal-checkout.nl/cartebleue">Carte Bleue</a> betalingen ontvangen binnen uw webshop.</p>';


	$aGatewaySettings = idealcheckout_getGatewaySettings(false, 'cartebleue');
	...
	if(!empty($aGatewaySettings['GATEWAY_VALIDATION']))
	{
		...
		<p>Controleer de status van alle openstaande Carte Bleue transacties bij uw Payment Service Provider.<br><br><input type="button" value="Controleer openstaande transacties." onclick="javascript: window.open(\'' . $sRootUrl . 'idealcheckout/validate.php?gateway=' . urlencode('cartebleue') . '\', \'popup\', \'directories=no,height=550,location=no,menubar=no,resizable=yes,scrollbars=yes,status=no,toolbar=no,width=750\');"></p>';
	}
