...
<html>
	...
	<body>
		...
		<p>Via deze plugin kunt u <a href="http://www.ideal-checkout.nl/mastercard">Mastercard</a> betalingen ontvangen binnen uw webshop.</p>';


	$aGatewaySettings = idealcheckout_getGatewaySettings(false, 'mastercard');
	...
	if(!empty($aGatewaySettings['GATEWAY_VALIDATION']))
	{
		...
		<p>Controleer de status van alle openstaande Mastercard transacties bij uw Payment Service Provider.<br><br><input type="button" value="Controleer openstaande transacties." onclick="javascript: window.open(\'' . $sRootUrl . 'idealcheckout/validate.php?gateway=' . urlencode('mastercard') . '\', \'popup\', \'directories=no,height=550,location=no,menubar=no,resizable=yes,scrollbars=yes,status=no,toolbar=no,width=750\');"></p>';
	}
