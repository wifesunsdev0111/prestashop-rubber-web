		...
		public function doSetup()
		{
			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					{
						$sSelectOptions = PayProApi::getIdealIssuerOptions(!empty($this->aSettings['TEST_MODE']));
...
<form action="' . htmlspecialchars(idealcheckout_getRootUrl(1) . 'idealcheckout/transaction.php?order_id=' . $sOrderId . '&order_code=' . $sOrderCode) . '" method="post" id="checkout">
	<p><b>Kies uw bank</b><br><select name="issuer_id" style="margin: 6px; width: 200px;">' . $sSelectOptions . '</select><br><input type="submit" value="Verder"></p>.
</form>';
					}
				}
				else
		...
		public function doTransaction()
		{
			...
			$aIdealIssuers = PayProApi::getIdealIssuers();

			// Look for proper GET's en POST's
			if(empty($_POST['issuer_id']) || !isset($aIdealIssuers[$_POST['issuer_id']]) || empty($_GET['order_id']) || empty($_GET['order_code']))
			{
			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					else
					{
						...
						$oApi->setParam('psp_code', $sIssuerId); // iDEAL
						...
						if(!empty($this->oRecord['order_params']))
						{
							...
							if(!empty($aOrderParams['customer']['payment_phone']))
							{
								$oApi->setParam('consumer_phoneno', $aOrderParams['customer']['payment_phone']);
							}
