...
<form action="' . htmlspecialchars(idealcheckout_getRootUrl(1) . 'idealcheckout/transaction.php?order_id=' . $sOrderId . '&order_code=' . $sOrderCode) . '" method="post" id="checkout">
	<p><b>Kies uw bank</b><br><select name="issuer_id" style="margin: 6px; width: 200px;"><script src="' . ((isset($_SERVER['HTTPS']) && (strcasecmp($_SERVER['HTTPS'], 'on') === 0)) ? 'https://' : 'http://') . 'www.targetpay.com/ideal/issuers-nl.js"></script></select><br><input type="submit" value="Verder"></p>.
		...
		public function doTransaction()
		{
			...
			{
				header('Location: ' . idealcheckout_getRootUrl(1) . 'idealcheckout/setup.php?order_id=' . urlencode($_GET['order_id']) . '&order_code=' . urlencode($_GET['order_code']));
				exit;
			}
		...
		public function doReturn()
		{
			...
			if(empty($_GET['ec']) && !empty($_POST['ec']))
			{
				$_GET['ec'] = $_POST['ec'];
			}
		...
		public function doValidate()
		{
			...
			if(idealcheckout_database_num_rows($oRecordset))
			{
				while($aRecord = idealcheckout_database_fetch_assoc($oRecordset))
				{
					// $oGateway = new TP_Ideal($this->aSettings['LAYOUT_CODE'], $this->oRecord['language_code'], $this->oRecord['country_code'], $this->oRecord['currency_code']);
					$oGateway = new TP_Ideal($this->aSettings['LAYOUT_CODE'], 'NL', 'NL', 'EUR');
					$sTransactionStatus = $oGateway->doStatusRequest($aRecord['transaction_id'], true, $this->aSettings['TEST_MODE']);
