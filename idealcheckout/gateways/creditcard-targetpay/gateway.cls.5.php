			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					else
					{
						...
						$oGateway = new TP_Creditcard($this->aSettings['LAYOUT_CODE'], $this->oRecord['language_code'], $this->oRecord['country_code'], $this->oRecord['currency_code']);
			...
			else
			{
				...
				if($this->getRecordByTransaction())
				{
					...
					if(strcmp($this->oRecord['transaction_status'], 'SUCCESS') === 0)
					{
						...
						{
							$sHtml .= '<p>' . idealcheckout_getTranslation(false, 'creditcard-targetpay', 'Payment was recieved') . '<br><input style="margin: 6px;" type="button" value="' . idealcheckout_getTranslation(false, 'creditcard-targetpay', 'Return to the website') . '" onclick="javascript: document.location.href = \'' . htmlspecialchars(idealcheckout_getRootUrl(1)) . '\'"></p>';
						}
					}
					else
					{
