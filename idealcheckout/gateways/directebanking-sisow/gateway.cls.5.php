<?php
	...
	class Gateway extends GatewayCore
	{
		...
		public function doSetup()
		{
			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					{
						$oSisow = new Sisow_Directebanking();
		...
		public function doValidate()
		{
			...
			$sql = "SELECT * FROM `" . $aIdealCheckout['database']['table'] . "` WHERE (`transaction_status` = 'OPEN') AND (`gateway_code` = '" . idealcheckout_escapeSql($aIdealCheckout['record']['gateway_code']) . "') AND (`store_code` = '" . idealcheckout_escapeSql($aIdealCheckout['record']['store_code']) . "') ORDER BY `id` ASC;";
