			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					else
					{
						...
						{
							idealcheckout_output('<code>' . var_export($oAssurePay->getErrors(), true) . '</code>');
						}
			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					else
					{
						...
						list($sTransactionId, $sTransactionUrl) = $oAssurePay->doTransactionRequest($sIssuerId, $this->oRecord['order_id'], $this->oRecord['transaction_amount'], $this->oRecord['transaction_description'], $this->oRecord['transaction_code'], idealcheckout_getRootUrl(1) . 'idealcheckout/return.php', idealcheckout_getRootUrl(1) . 'idealcheckout/report.php');
						...
						// idealcheckout_die('<a href="' . $oAssurePay->getTransactionUrl() . '">' . $oAssurePay->getTransactionUrl() . '</a>', __FILE__, __LINE__);
		...
		public function doValidate()
		{
			...
			if(idealcheckout_database_num_rows($oRecordset))
			{
				while($aRecord = idealcheckout_database_fetch_assoc($oRecordset))
				{
					...
					$aRecord['transaction_status'] = $oAssurePay->doStatusRequest($aRecord['transaction_id']);
