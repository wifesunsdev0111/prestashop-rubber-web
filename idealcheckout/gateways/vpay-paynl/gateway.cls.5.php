			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					else
					{
						...
						$oPaynl->setPaymentMethod('VPAY');
						$oPaynl->setReturnUrl(idealcheckout_getRootUrl(1) . 'idealcheckout/return.php?order_id=' . urlencode($sOrderId) . '&order_code=' . urlencode($sOrderCode));
						$oPaynl->setReportUrl(idealcheckout_getRootUrl(1) . 'idealcheckout/report.php?order_id=' . urlencode($sOrderId) . '&order_code=' . urlencode($sOrderCode));
		...
		public function doValidate()
		{
			...
			if(idealcheckout_database_num_rows($oRecordset))
			{
				while($aRecord = idealcheckout_database_fetch_assoc($oRecordset))
				{
					...
					$this->oRecord['transaction_status'] = $oPaynl->doStatusRequest($aRecord['transaction_id']);
