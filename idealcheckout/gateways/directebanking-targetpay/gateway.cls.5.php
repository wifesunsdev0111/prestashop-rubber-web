<?php
	...
	class Gateway extends GatewayCore
	{
		...
		public function doSetup()
		{
			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					{
						$sReturnUrl = str_replace(array(' '), array('%20'), idealcheckout_getRootUrl(1)) . 'idealcheckout/return.php?transaction_code=' . urlencode($this->oRecord['transaction_code']);
						$sReportUrl = str_replace(array(' '), array('%20'), idealcheckout_getRootUrl(1)) . 'idealcheckout/report.php?transaction_code=' . urlencode($this->oRecord['transaction_code']);

						$oGateway = new TP_Directebanking($this->aSettings['LAYOUT_CODE'], $this->oRecord['language_code'], $this->oRecord['country_code'], $this->oRecord['currency_code']);
						$aTransactionData = $oGateway->doTransactionRequest($this->oRecord['transaction_amount'], $this->oRecord['transaction_description'], $sReturnUrl, $sReportUrl);
						...
						if(is_array($aTransactionData))
						{
							...
							// idealcheckout_die('<a href="' . $aTransactionData['transaction_url'] . '">' . $aTransactionData['transaction_url'] . '</a>', __FILE__, __LINE__);
		...
		public function doReturn()
		{
			...
			if(empty($_GET['trxid']) && !empty($_POST['trxid']))
			{
				$_GET['trxid'] = $_POST['trxid'];
			}

			if(empty($_GET['transaction_code']) && !empty($_POST['transaction_code']))
			{
				$_GET['transaction_code'] = $_POST['transaction_code'];
			}

			if(empty($_GET['trxid']) || empty($_GET['transaction_code']))
			{
			...
			else
			{
				...
				if($this->getRecordByTransaction())
				{
					...
					if(strcmp($this->oRecord['transaction_status'], 'SUCCESS') === 0)
					{
						...
						{
							$sHtml .= '<p>' . idealcheckout_getTranslation(false, 'directebanking-targetpay', 'Payment was recieved') . '<br><input style="margin: 6px;" type="button" value="' . idealcheckout_getTranslation(false, 'directebanking-targetpay', 'Return to the website') . '" onclick="javascript: document.location.href = \'' . htmlspecialchars(idealcheckout_getRootUrl(1)) . '\'"></p>';
						}
					}
					else
					{
