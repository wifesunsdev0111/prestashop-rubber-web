	...
	*/
	...
	class Sisow_Ideal
	{
		...
		public function __construct()
		{
			...
			if(defined('SISOW_CACHE_PATH'))
			{
				$this->setCachePath(SISOW_CACHE_PATH);
			}
		}
			...
			if($sXmlReply)
			{
				if($this->parseFromXml('directory', $sXmlReply))
				{
					...
					if($sCacheFile)
					{
						...
						{
							fwrite($oHandle, idealcheckout_serialize($this->aIssuers));
							fclose($oHandle);
						}
					}
				}
		...
		// Return Issuers
		public function getIssuers($bHtml = false)
		{
			if($bHtml)
			{
				$sIssuers = '';
		...
		protected function postToHost($sUrl, $sPostData = false, $bRemoveHeaders = false, $iTimeout = 30)
		{
			...
			{
				idealcheckout_die('Socket error: ' . $sErrorMessage);
			}
