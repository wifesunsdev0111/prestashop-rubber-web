	...
	class IdealInternetKassa
	{
		...
		protected function getActionUrl()
		{
			if(isset($this->aValues['ACQUIRER']))
			{
				...
				{
					return 'https://internetkassa.abnamro.nl/ncol/' . ($this->aValues['TEST_MODE'] ? 'test' : 'prod') . '/orderstandard.asp';
				}
				elseif(strpos($sAcquirer, 'fortis') !== false) // Fortis Bank (Neos Solutions)
				{
					return 'https://www.secure.neos-solution.com/ncol/' . ($this->aValues['TEST_MODE'] ? 'test' : 'prod') . '/orderstandard.asp';
				}
		...
		public function createForm($sSubmitLabel = 'Afrekenen')
		{
			if(!empty($this->aValues['SHA1_IN_KEY']))
			{
				...
				$sHash = strtoupper(hash('sha512', $sHashString));
			}
...
<form method="post" action="' . $this->getActionUrl() . '" id="ogone" name="ogone">';
	...
	<input type="submit" value="' . htmlspecialchars($sSubmitLabel) . '">
		...
		public function validate()
		{
			...
			if(isset($_GET))
			{
				if(!empty($this->aValues['SHA1_OUT_KEY']))
				{
				...
				{
					$sTransactionStatus = strtoupper($_GET['status']);

					if(!in_array($sTransactionStatus, array('SUCCESS', 'PENDING', 'CANCELLED', 'FAILURE')))
					{
						return false;
					}
				}
