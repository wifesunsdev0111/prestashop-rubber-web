			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					else
					{
						...
						$sHtml = '<p><b>' . idealcheckout_getTranslation(false, 'idealcheckout', 'Checkout with {0}', array('iDEAL')) . '</b></p>' . $oOmniKassa->createForm();
			...
			else
			{
				...
				if($aOmniKassaResponse && is_array($aOmniKassaResponse))
				{
					...
					if($this->oRecord = idealcheckout_database_getRecord($sql))
					{
						...
						elseif((strcmp($this->oRecord['transaction_status'], $aOmniKassaResponse['transaction_status']) !== 0) && !in_array($this->oRecord['transaction_status'], array('SUCCESS', 'CANCELLED')))
						{
