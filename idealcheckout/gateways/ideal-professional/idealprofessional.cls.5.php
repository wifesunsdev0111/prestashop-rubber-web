	...
	*/
	...
	class IdealRequest
	{
		...
		public function setAquirer($sAquirerName, $bTestMode = false)
		{
			...
			elseif((strpos($sAquirerName, 'abn') !== false) && (strpos($sAquirerName, 'integrated') !== false)) // ABN AMRO - iDEAL Integrated
			{
				$this->sPublicCertificateFile = 'abnamro.integrated.cer';
				$this->sAquirerUrl = 'ssl://abnamro' . ($bTestMode ? '-test' : '') . '.ideal-payment.de:443/ideal/iDeal';
			}
			elseif(strpos($sAquirerName, 'abn') !== false) // ABN AMRO - iDEAL Zelfbouw
			{
				$this->bABNAMRO = true;
		...
		// Get fingerprints for all public certificates
		protected function getPublicFingerprints()
		{
			$aSignatures = array();
			...
			// Upto 10 public certificates; eg: rabobank-0.cer, rabobank-1.cer, rabobank-2.cer, etc.
			for($i = 0; $i < 10; $i++)
			{
			...
			{
				if($fp = fopen($this->sSecurePath . $sCertificateFile, 'r'))
				{
					$sRawData = fread($fp, 8192);
					fclose($fp);
					...
					if(!openssl_x509_export($sData, $sData))
					{
						$this->setError('Error in certificate ' . $this->sSecurePath . $sCertificateFile, false, __FILE__, __LINE__);
						return false;
					}
					...
					$aSignatures[] = strtoupper(sha1(base64_decode($sData)));
				}
			}
		...
		// Compare signature with all public certificates
		protected function verifyPublicSignature($sData, $sSignature)
		{
			$aSignatures = array();
			$aCertificateFiles = array();
			...
			foreach($aCertificateFiles as $sCertificateFile)
			{
				if(($bOk === false) && ($fp = fopen($this->sSecurePath . $sCertificateFile, 'r')))
				{
					$sRawData = fread($fp, 8192);
					fclose($fp);
					...
					if($sPublicKey = openssl_get_publickey($sRawData))
					{
						if(openssl_verify($sData, $sSignature, $sPublicKey));
						{
							$bOk = true;
						}
					...
					{
						$this->setError('Cannot retrieve key from public certificate file: ' . $sCertificateFile, false, __FILE__, __LINE__);
					}
				}
			}
				...
				if($sXmlReply)
				{
					...
					else
					{
						...
						if($sCacheFile)
						{
							...
							{
								fwrite($oHandle, idealcheckout_serialize($aIssuerList));
								fclose($oHandle);
							}
						}
			...
			if($this->checkConfiguration() && $this->checkConfiguration(array('sTransactionId')))
			{
				...
				if($sXmlReply)
				{
					...
					else
					{
						...
						$aPublicSignatures = $this->getPublicFingerprints();

						if(!is_array($aPublicSignatures) || !in_array(strtoupper($sFingerprint), $aPublicSignatures))
						{
							// Invalid Fingerprint
							$this->setError('Unknown fingerprint.', false, __FILE__, __LINE__);
						}
						elseif($this->verifyPublicSignature($sMessage, $sSignature) == false)
						{
							// Invalid Fingerprint
